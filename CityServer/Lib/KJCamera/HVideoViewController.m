...
- (IBAction)onEnsureAction:(UIButton *)sender {
    ...
    if (self.saveVideoUrl) {
        ...
        MPMoviePlayerController *player = [[MPMoviePlayerController alloc]initWithContentURL:self.saveVideoUrl] ;
        ...
        model.videoURL = self.saveVideoUrl;
        ...
        model.thumbPhoto = self.takeImage;
        model.previewPhoto = self.takeImage;
...
- (void)onStartTranscribe:(NSURL *)fileURL {
    if ([self.captureMovieFileOutput isRecording]) {
        -- self.seconds;
        if (self.seconds > 0) {
            if (self.HSeconds - self.seconds <= 3 && !self.isVideo) {
                NSLog(@"视频不能少于3秒");
            }else if (self.HSeconds - self.seconds >= TimeMax && !self.isVideo) {
